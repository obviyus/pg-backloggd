...
class AllCategoriesSpider(scrapy.Spider):
    ...
    def __init__(self, username=None, *args, **kwargs):
        ...
        # Temporary set robots.txt rules
    ...
    def parse(self, response):
        ...
        #Extract the status based on the URL type
        ...
        #Extract the game, rating and store data
        for card in response.css('div.card.mx-auto.game-cover'):
            ...
            data = {'Game': game, 'Status': status, 'Rating': rating}
            self.results.append(data)
            self.log(f"Scraped: Game: {game}, Status: {status}, Rating: {rating}")

        #Check for next page exists to retrieve from
        next_page = response.css('a[rel="next"]::attr(href)').get()
        if next_page:
            self.log(f"Following next page: {urljoin(response.url, next_page)}")
            yield scrapy.Request(url=urljoin(response.url, next_page), callback=self.parse, errback=self.handle_error)
    ...
    def scrape_journal_data(self, url):
        ...
        for entry in entries:
            ...
            game_name = game_name_elem.find('a').text.strip() if game_name_elem else None
            ...
            if cleaned_game_name not in seen_games:
                ...
                formatted_game_name = re.sub(r'[^a-zA-Z0-9]', '-', cleaned_game_name.lower())
    ...
    def scrape_played_dates(self, game_url):
        ...
        for candidate in date_elem_candidates:
            ...
            if date_elem:
                status_elem = candidate.find_next('div', class_='col-auto col-md-2 my-auto ml-auto order-md-last')
        ...
        if last_played_date_elem:
            ...
            if last_played_elem:
                last_played_elem = last_played_elem.find('p', class_='date-tooltip right-tooltip')

            last_played_date = last_played_elem.text.strip() if last_played_elem else None
    ...
    def combine_and_export_to_csv(self):
        csv_filename = 'all_entries.csv'
        fieldnames = ['Game', 'Status', 'Rating', 'Start date', 'Finish date']

        #Load the journal data
        journal_data = self.scrape_journal_data(f'https://www.backloggd.com/u/{username}/journal/')

        #Combine the data based on the game title
        combined_data = []
        for category_entry in self.results:
            #Check if the game title exists in the journal data
            journal_entry = next((entry for entry in journal_data if entry['Game Name'] == category_entry['Game']), None)
